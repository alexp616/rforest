/*
  Test that cu_mpz_rmatrix_mult_fft produces the same result as
  mpz_rmatrix_mult_naive (the latter being a trivial O(rd^2)
  implementation used as a ground-truth reference).

  Usage:  ./test_gpu_matmul [bits] [d] [r]
...
int main(int argc, char *argv[])
{
    ...
    printf("Testing cu_mpz_rmatrix_mult_fft vs mpz_rmatrix_mult_naive\n");
    ...
    mpz_t *C_gpu = mpz_vec_alloc_and_init(r * d);   /* result from gpu fft */
    ...
    /* --- Compute: C_gpu = A*B  (gpu fft, exact) --- */
    cu_mpz_rmatrix_mult_fft(C_gpu, A, r, B, d, w);

    /* --- Compare --- */
    int ok = 1;
    for (int i = 0; i < r * d; i++) {
        if (mpz_cmp(C_gpu[i], C_ref[i]) != 0) {
    ...
    mpz_vec_clear_and_free(C_gpu, r * d);
