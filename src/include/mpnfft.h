...
// ---- GPU (cu_) variants using flat contiguous layout ----

// GPU version of params init: sets points = N (power-of-2)
void cu_zz_mpnfft_params_init(zz_mpnfft_params_t* params, size_t bits,
			      size_t terms, unsigned num_primes,
			      zz_moduli_t* moduli);

// Converts integer {up,un} to flat polynomial layout for GPU
void cu_zz_mpnfft_mpn_to_poly(uint64_t** dest, zz_mpnfft_params_t* params,
			      mp_limb_t* up, size_t un,
			      int sign, int lgS, int scale, int threads);

// Writes f(2^r) mod B^rn to mpz_t rop from flat layout
void cu_zz_mpnfft_poly_to_mpn(mpz_t rop, size_t rn, uint64_t** data,
			      zz_mpnfft_params_t* params, int threads);

// GPU batch FFT: returns device pointer
uint64_t* cu_zz_mpnfft_poly_fft(uint64_t* data, zz_mpnfft_params_t* params,
				int datasz);

// GPU batch inverse FFT
void cu_zz_mpnfft_poly_ifft(uint64_t* host_ptr, uint64_t* data,
			    zz_mpnfft_params_t* params, int datasz);

// GPU matrix multiply of Fourier coefficients
void cu_zz_mpnfft_poly_matrix_mul(uint64_t* C, uint64_t* A, uint64_t* B,
