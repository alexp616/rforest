			      ...
			      zz_moduli_t* moduli)
{
  ...
  while (1)
      ...
      if (rr == r)
	{
	  ...
	  params->lgN = lgN;
	  params->N = (size_t) 1 << lgN;
	  params->points = params->N;   // GPU needs power-of-2 sizes
			      ...
			      zz_mpnfft_params_t* params, int threads)
{
  ...
  while (rn > 0 && rop->_mp_d[rn - 1] == 0)
    rn--;
  rop->_mp_size = neg ? -(int)rn : (int)rn;
}
				 ...
				 int datasz)
{
  ...
  // Reduce inputs from [0, 2p) to [0, p) before GPU NTT
  for (int i = 0; i < num_primes; i++) {
    uint64_t p = params->moduli->p[i];
    uint64_t* ptr = data + (size_t)i * datasz;
    for (size_t j = 0; j < (size_t)datasz; j++) {
      if (ptr[j] >= p) ptr[j] -= p;
    }
  }
